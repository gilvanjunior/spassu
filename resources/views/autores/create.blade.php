...
@section('content_header')
    <h1>{{ $autor ? 'Alterar' : 'Cadastrar' }} Autor</h1>
@stop
    ...
    <x-adminlte-card class="shadow">
        ...
        <form action="" id="form_cadastrar_atualizar" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <x-adminlte-input type="text" name="nome" id="nome" label="Nome" placeholder="Nome do autor" enable-old-support></x-adminlte-input>
                </div>
            </div>
            @if($autor)
            ...
            @endif   
            <x-adminlte-button label="Cancelar" icon="fas fa-ban" onclick="location.href='/autor/listar'" />
        </form>
    </x-adminlte-card>
...
@section('js')
    <script>
        let nome = "{{ $autor ? $autor->nome : null }}";
    </script>
